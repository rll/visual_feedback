...
class PassiveShapeMaker:
    ...
    def process_image(self,thresh):
        ...
        if CONTOURS_ONLY:
            cv.ShowImage("Result",self.image_out)
            return
        #cv.ShowImage("Result",self.image_out)
        #return
        ...
        if SHOW_UNSCALED_MODEL:
            self.model.draw_to_image(self.image_out,cv.CV_RGB(0,0,255))
            cv.ShowImage("Result",self.image_out)
            return
        ...
        if SHOW_SYMM_MODEL:
            new_model_symm.draw_to_image(img=self.image_out,color=cv.CV_RGB(0,255,0))
        ...
        final_model.draw_to_image(img=self.image_out,color=cv.CV_RGB(255,0,255))
        nearest_pts = []
        ...
        if SHOW_FITTED:
            ...
            fitted_model.draw_to_image(img=self.image_out,color=cv.CV_RGB(0,255,255))
        if ANNOTATE:
    ...
    def energy_fxn(self,model,contour):
        ...
        #Normalize
        model_dist_energy /= float(self.dist_fxn(max(self.image_out.width,self.image_out.height)))
        ...
        #Normalize
        contour_dist_energy /= float(self.dist_fxn(max(self.image_out.width,self.image_out.height)))
    ...
    def highlight_pt(self,pt,color=None):
        ...
        cv.Circle(self.image_out,pt,5,color,3)
